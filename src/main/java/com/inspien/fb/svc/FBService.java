...
public class FBService{
	...
	private static DecimalFormat longFormatter = new DecimalFormat("000000000000"); //12자리의 10진수의 decimalformat객체를 생성한다.
	...
	public long getCounter(String orgCode) { //orgCode 기관 넘버를 받는다.
		...
		if(custCounter.containsKey(orgCode)) { //orgCode가 이미 custCounter에 key로 존재하고,
			if(custCounter.get(orgCode).containsKey(today)) { //orgCode 키에 대한 value값에 today가 포함된다면
				txNo = custCounter.get(orgCode).get(today).incrementAndGet(); //txNo는 today값에 포함된 txNo에 접근하여 increment한 후 Get을 한다.
			}
			else {
	...
	public TransferResponse transfer(TransferRequest req) throws Exception {
		...
		long txNo = getCounter(req.getOrg_code()); //org_code를 인자로 받아서 getCounter함수 실행
		...
		// call VAN API
		if (txNo == 1) { //txNo가 1이면, 먼저 https://localhost:9000/firmapi/rt/v1/ 해당 엔트리로 보내져 최초로 실행될때는 txNo에 대한 증감연산이 없으므로
						// txNo가 1이 반환된다.
			OpenRequest openReq = OpenRequest.builder() //게시전문에 대하여 요청할 데이터를 세팅한다.
					...
					.drw_bank_code(req.getDrw_bank_code()) //OpenReq의 멤버변수 drw_bank_code는 req(요청데이터)의 Drw_bank_code속성을 getter로 가져와 세팅
		...
		try {
			txNo = getCounter(req.getOrg_code()); //org_code를 인자로 받아서 getCounter함수 실행, 두번째 실행이므로 게시전문이 이미 있으므로, 출력이 2로 세팅될것
